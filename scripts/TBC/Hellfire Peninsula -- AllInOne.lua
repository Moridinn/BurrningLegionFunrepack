...
function CollapsingVoidwalker_OnEnterCombat(pUnit,Event)
	pUnit:RegsiterEvent("CollapsingVoidwalker_Fear", 14000, 0)
end
...
function CollapsingVoidwalker_OnDied(pUnit,Event)
	pUnit:CastSpell(34302)
	pUnit:CastSpell(34233)
	pUnit:FullCastSpellOnTarget(34234,pUnit:GetClosestPlayer())
	pUnit:CastSpell(34842)
	pUnit:RemoveEvents()
end
...
function CursedScarab_OnSpawn(pUnit,Event)
	pUnit:FullCastSpellOnTarget(3256)
	pUnit:RegisterEvent("CursedScarab_OnDespawn", 20000, 1)
end

function CursedScarab_OnDespawn(pUnit,Event)
	pUnit:Despawn()
end

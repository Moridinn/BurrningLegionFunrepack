...
function BigOoze_UnstableOozeExplosion(punit, event)
  if bigooze:HasAura(69558) == 5 then
    local target = bigooze:GetRandomPlayer(0)
      bigooze:FullCastSpellOnTarget(69839, target)
      bigooze:FullCastSpellOnTarget(69839, target)
      bigooze:FullCastSpellOnTarget(69839, target)
	    local targettarget = target:GetClosestFriend()
	   if targettarget:GetDistanceYards(target) < 6 then
	    target:CastSpellOnTarget(69839, targettarget)
		 rotface:SendChatMessage(14, 0, "I think I made an angry poo-poo. It gonna blow!")
	  end
   end
end
